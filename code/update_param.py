...
def rho(p=0.0, temp=0.0):
    ...
    dt = temp - temp_ref  # np.clip(temp-temp_ref, a_min=-temp_ref/6, a_max=temp_ref/6)

    density = rho_f * (1 + c * (p - p_ref) - beta * dt)
...
def update_mass_weight(mdg, iterate=True):
    ...
    for sd, d in mdg.subdomains(return_data=True):
        ...
        heat_capacity = (
            ...
            + (1 - porosity.copy())  # fluid part
            * solid_density
            * specific_heat_capacity_solid  # solid poart
        )
        conduction = (
