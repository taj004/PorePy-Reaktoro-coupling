...
# Define mdg
sd = pp.CartGrid(nx=[100], physdims=[1])
sd.compute_geometry()
...
aq_species = [
    ...
    "SiO2(aq)" # added to acount for the kinetic representation of quartz
]
...
chem_system = rt.ChemicalSystem(
    ...
    rt.MineralReaction("Calcite").setRateModel(
    ...
    rt.MineralReaction("Dolomite").setRateModel(
        rt.ReactionRateModelPalandriKharaka(params)
        ),
    
    rt.MineralSurface("Calcite",  6.0, "cm2/cm3"),
    rt.MineralSurface("Quartz",   6.0, "cm2/cm3"),
    rt.MineralSurface("Dolomite", 6.0, "cm2/cm3")
...
chemical_solver.state.set("Calcite" , 1.0, "mol")
chemical_solver.state.set("Quartz"  , 1.0, "mol")  

#%% Set pressure and temperature, and initialise chemical states
...
init_conv = chemical_solver.solve_chemistry(dt=1800) 
...
# Scale the aqueous phase
chemical_solver.state.scalePhaseVolume("AqueousPhase", porosity, "m3")
...
# aq_init_props = rt.AqueousProps(chemical_solver.state)
# init_ph = aq_init_props.pH()
...
aq_bc_props = rt.AqueousProps(bc_chemistry.state)
bc_ph = aq_bc_props.pH()
...
for sd, d in mdg.subdomains(return_data=True):
    ...
    # Set bc values. Note, at each face we have num_components
...
# Make folder
folder_name = "pictures/simulation_2/"
...
#     conc_name = ["CO2(aq)", "Ca+2", "HCO3-", "OH-", "H+", "Mg(HCO3)+", "SiO2(aq)"]
#     colour_name = ["b", "purple" ,"violet", "r", "g","k", "cyan"]
#     label_name = ["CO$_{2}$(aq)", "Ca$^{2+}$", "HCO$_{3}^{-}$", 
#                   "OH$^{-}$", "H$^{+}$", "Mg(CO$_{3}$)$^{+}$", "SiO$_{2}$(aq)" ]
...
def plot_1d_several(title_time, index):
    ...
    if index == 2:
        ax.legend(loc="center right", fontsize=20, bbox_to_anchor=(1.77, 0.70))
    # else
...
#%% Time loop
store_time=np.array([30*pp.MINUTE, 10*pp.HOUR, 50*pp.DAY])
store_title_time = ["30 Minutes", "10 Hours", "50 Days"]
j=0
...
# Save the 2x3 figure
plt.savefig(
    folder_name+"conc_mineral_kin_in_one.pdf", 
    bbox_inches="tight", 
    pad_inches=0.1
)
plt.show() 
